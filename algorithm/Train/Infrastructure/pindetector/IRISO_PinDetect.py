...
'''1217'''
def detect_pin(image_gray,  pincount=35, offset_up=20, offset_down=40, pinsizethreshold=200, errsizethreshold=30):
    # 初始化返回值
    errBoxs = []
    # 取得图像信息
...
def draw_pin_overlay(base_bgr: np.ndarray, err_boxes: List[Box]) -> np.ndarray:
    """把 errBoxs 画到原图上（叠加框）"""
    out = base_bgr.copy()
    for (x1, y1), (x2, y2) in err_boxes:
        cv2.rectangle(out, (x1, y1), (x2, y2), (0, 0, 255), 2)
    return out


def defect_count(err_boxes: List[Box]) -> int:
    return int(len(err_boxes))

if __name__ == "__main__":
    # 搜索当前文件夹
    pattern = os.path.join(r"D:\IRISO\1204\NG", "*.jpg")
    ...
    for index, image_path in enumerate(jpg_files):
        ...
        image_org = cv2.imread(rf"D:\IRISO\1204\NG\{filename}.jpg", cv2.IMREAD_COLOR_BGR)
        ...
        # 针脚检测
        start = time.time()
        image_pin, errBoxs = detect_pin(image_gray, pincount=35, offset_up=50, offset_down=80)
